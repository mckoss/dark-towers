...
import { assessRequest, towerHoursText, type NasrAirport } from '$lib/nasr';
...
export const actions: Actions = {
	lookup: async ({ request }) => {
		...
		const result = lookupCandidate(value);
		if (!result.candidate) return fail(400, { error: result.error, value });

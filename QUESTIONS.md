...
## Backfill / history
21. When you upgrade the AeroAPI tier, set `"aeroapi_history": true` in `config.json` (and `CONFIG_JSON`). Then `/admin → Backfill` (or `npm run ingest -- PAE --backfill 120`) fills older nights; previously recorded "too old" misses are retried automatically. For a newly approved airport: flip `tracked: true` in `src/lib/airports.ts`, deploy, then Backfill 30 nights from `/admin`. Want `history_days` raised to 30 at the same time so the hourly scheduler self-heals deeper gaps? (Costs nothing once nights are complete.)
